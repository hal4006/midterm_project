...
ch4.e <- total.ch4 / total.co2 * co2.emission
no2.e <- total.no2 / total.co2 * co2.emission
## Set a function to add environment impact to data frame
numeric_format <- function(x) {
    as.numeric(round(x,2))
}
enviroment_impact <- function(df, var) {
    df <- df %>%
...
shinyServer(function(input, output) {
    ...
    pack_summary <- reactive({
        ...
        enviroment_impact(pack_heads, pack_heads[,5])
    })
    

    output$total_heads <- renderTable({
        min_head <- min(as.numeric(pack_summary()[,5]))
        max_head <- max(as.numeric(pack_summary()[,5]))
        data.frame(Name = c("Actual Heads in Need", "Minimum Heads"),
                   Value = c(max_head, min_head))
    })
